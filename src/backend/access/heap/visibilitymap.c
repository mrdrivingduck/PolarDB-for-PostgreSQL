						   ...
						   XLogReaderState *polar_record)
{
	...
	Assert(BufferIsLocal(vmbuf) || BufferIsExclusiveLocked(vmbuf));
	...
	if (map[mapByte] & mask)
	{
		...
		if (polar_enable_shared_storage_mode)
		{
			...
			if (polar_record == NULL)
			{
				...
				PageSetLSN(BufferGetPage(vmbuf), polar_get_fake_latest_lsn());
			}
			/* System is in recovery, set the oldest lsn and latest lsn */
			else
			{
				Assert(RecoveryInProgress());
				PageSetLSN(BufferGetPage(vmbuf), polar_record->EndRecPtr);
				polar_redo_set_buffer_oldest_lsn(vmbuf, polar_record->ReadRecPtr);
			}
		}
		/* POLAR end */
...
visibilitymap_get_status(Relation rel, BlockNumber heapBlk, Buffer *vmbuf)
{
	 ...
	 * than replay lsn. Acquire XLogCtl->lastReplayedEndRecPtr without lock to
	 * avoid lock contention between backend and startup, otherwise it will
	 * increase replay lag when accessing the vm very frequently.
	 */
	...
	if (is_replica_or_promoting &&
		PageGetLSN(BufferGetPage(*vmbuf)) > polar_get_xlog_replay_recptr_nolock())
		return false;
...
vm_readbuf(Relation rel, BlockNumber blkno, bool extend)
{
	...
	if (blkno >= reln->smgr_cached_nblocks[VISIBILITYMAP_FORKNUM])
	{
		if (extend)
		{
			 ...
			 *
			 * For replica, rel size cache should be invalided to update.
			 */
			if (!polar_is_replica())
			...
			else
			{
				...
				if (POLAR_RSC_REPLICA_ENABLED())
					polar_rsc_drop_entry(&reln->smgr_rlocator.locator);
				/* POLAR end */
