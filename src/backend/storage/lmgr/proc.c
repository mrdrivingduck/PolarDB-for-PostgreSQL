...
InitProcess(void)
{
	...
	/* POLAR */
	MyProc->polar_master_pgprocno = MyProc->pgprocno;
...
ProcKill(int code, Datum arg)
{
	...
	if (push_leader)
	{
		...
		PGPROC	   *volatile *leadergloballist = leader->procgloballist;

		leader->links.next = (SHM_QUEUE *) *leadergloballist;
		*leadergloballist = leader;
	}
	if (push_self)
	{
