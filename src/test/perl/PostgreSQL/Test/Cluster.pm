...
use Socket;
use Test::More;
use PostgreSQL::Test::Utils          qw(polar_die polar_note);
...
sub init
{
	...
	else
	{
		...
		my $template_dir = $ENV{INITDB_TEMPLATE};
		if (defined $params{init_polar_node} and $params{init_polar_node})
		{
		...
		@copycmd = (@copycmd, $template_dir, $pgdata);
	...
	print $conf "polar_enable_switch_wal_in_backup = on\n";
...
sub interactive_psql
{
	...
	if (defined $params{die_after_timeout})
	{
		$die_after_timeout = $params{die_after_timeout};
	}
...
sub pgbench_test
{
	...
	if (defined $params{files} && defined $params{script})
	{
		die "you cannot specify files and script together";
	}
	# run with -f file.sql
	elsif (defined $params{files})
	{
		$workload = join(' ', $self->_pgbench_make_files($params{files}));
	}
	# run with builtin workload -b xxx
	elsif (defined $params{script} && $params{script} eq "none")
	{
		note "pgbench without builtin scripts";
	}
	else
	{
		$params{script} = 'tpcb-like' unless defined $params{script};
		$workload = "-b " . $params{script};
	}
...
sub polar_default_conf
{
	...
	# turn on to avoid PANIC during restart of RO/Standby
	allow_in_place_tablespaces=on
	polar_has_partial_write=on
	vacuum_buffer_usage_limit=1MB
	polar_fullpage_snapshot_oldest_lsn_delay_threshold = 8
	polar_fullpage_snapshot_replay_delay_threshold = 1
	polar_fullpage_snapshot_min_modified_count = 2
	polar_enable_parameters_inconsistency=on
	polar_primary_parallel_replay_mode='async_instant_recovery'
...
sub polar_create_slot
{
	...
	if (defined $params{type} and $params{type} eq 'logical')
	{
		$self->psql(
			'postgres',
	...
	else
	{
		...
		if (defined $params{immediately_reserve})
		{
			$reserve = $params{immediately_reserve};
		}

		$self->psql(
			'postgres',
			"SELECT * FROM pg_create_physical_replication_slot('$slot', '$reserve')",
			on_error_die => 1,
			on_error_stop => 1);
	}
}
...
sub psql_connect
{
	...
	open my $OUTPUT, '+>', $psql->{_psql_output_file}
	  or polar_die "failed to open $psql->{_psql_output_file}: $!";
	truncate($OUTPUT, 0)
	  or polar_die "failed to truncate $psql->{_psql_output_file}: $!";
	close $OUTPUT;
	...
	polar_die "psql_connect timed out, stderr:\n$psql->{_psql_stderr}\n"
...
sub psql_fetch_result
{
	...
	open my $OUTPUT, '<', $psql_output_file
	  or polar_die "faile $psql_output_file open failed!";
	...
	polar_die
	  "Unexpected prev_size = $prev_size, cur_size = $cur_size, length = "
	  . length($psql_stdout)
...
sub psql_step_forward
{
	...
	polar_die "Only support single step mode"
	  if ($psql->{single_step_mode} ne 1);
	...
	if ($timer->is_expired)
	{
		polar_note "##$$'s psql process exited!\n"
		  . "Timer($timeout): "
		  . $timer->is_expired . "\n"
		  . "psql_stderr is:\n$psql->{_psql_stderr}\n"
		  . "psql_input is:\n$psql->{_psql_input}\n";
...
sub psql_execute
{
	...
	polar_die "Not support single step mode"
	  if ($psql->{single_step_mode} eq 1);

	# build input sql file
	my $input_file = "$PostgreSQL::Test::Utils::log_path/$$.sql";
	open my $INPUT, '+>', $input_file
	  or polar_die "failed to open $input_file open: $!";
	...
	{
		polar_note "##$$'s psql process exited! SQL is:\n$sql\n"
		  . "Timer($timeout): "
		  . $timer->is_expired . "\n"
		  . "psql_stderr is:\n$psql->{_psql_stderr}\n"
		  . "psql_input is:\n$psql->{_psql_input}\n";
...
=item $node->polar_is_in_recovery()

=cut

sub polar_is_in_recovery
{
	my ($self) = @_;
	my $ret = 0;
	my ($stdout, $stderr) = ('', '');
...
sub generate_wal
{
	...
	while (1)
	{
		...
		my $diff = $self->safe_psql('postgres',
			"SELECT diff from pg_wal_lsn_diff('$cur_insert_lsn'::pg_lsn, '$start_lsn'::pg_lsn) as diff;"
		);
		if ($diff > $size)
		{
			note "create WAL records [$start_lsn, $cur_insert_lsn)\n";
			return $diff;
		}
		note
		  "current insert lsn: $cur_insert_lsn, start insert lsn: $start_lsn\n";
	}
}
