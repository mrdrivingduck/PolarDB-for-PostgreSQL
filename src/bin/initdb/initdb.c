...
static const char *const valid_deploy_modes[] = {
	"cloud",
	"apclocal",
	"apcshared",
	NULL
...
/* POLAR: parameters manage */
static const char *deploy_mode = NULL;
...
static char *polar_settings_file = NULL;
...
static void setup_polar_settings(bool according_to_template);
...
static void load_default_extension(FILE *cmdfd);
...
/*
 * set up polar_settings.conf config file.
 */
static void
setup_polar_settings(bool according_to_template)
{
	char	  **conflines;
	char		path[MAXPGPATH];

	/* POLAR: Skip init when no template */
	if (according_to_template &&
		polar_settings_file == NULL)
		return;

	/* POLAR: init polar_settings.conf */
	snprintf(path, sizeof(path), "%s/polar_settings.conf", pg_data);

	/*
	 * Use parameters template to init polar_settings.conf when deploy-mode is
	 * specified
	 */
	if (according_to_template &&
		polar_settings_file != NULL)
	{
		fputs(_("creating polar_settings configuration file ... "), stdout);
		fflush(stdout);

		conflines = readfile(polar_settings_file);

		writefile(path, conflines);
	}

	/*
	 * Use empty file to init polar_settings.conf like postgresql.auto.conf
	 * when no according to template.
	 */
	else
	{
		conflines = pg_malloc_array(char *, 3);
		conflines[0] = pg_strdup("# Do not edit this file manually!\n");
		conflines[1] = pg_strdup("# It will be overwritten by the ALTER SYSTEM FOR CLUSTER command.\n");
		conflines[2] = NULL;

		writefile(path, conflines);
	}

	if (polar_chmod(path, pg_file_create_mode) != 0)
	{
...
setup_config(void)
{
...
#ifndef USE_TDE
		pg_log_error("cluster encryption is not supported because TDE is not supported by this build. Compile with --with-tde to use cluster encryption.");
		exit(1);
#endif
	...
	/* POLAR: initialization polar_settings.conf like postgresql.auto.conf */
	setup_polar_settings(false);
	/* POLAR */
...
bootstrap_template1(void)
{
	...
	if (enc_cipher)
		appendPQExpBuffer(&cmd, " -e %s", enc_cipher);
	if (debug)
...
static void
load_default_extension(FILE *cmdfd)
{
	PG_CMD_PUTS("CREATE EXTENSION pg_cron_preload;\n\n");
	PG_CMD_PUTS("CREATE EXTENSION polar_feature_utils;\n\n");
}
...
make_postgres(FILE *cmdfd)
{
	...
	/*
	 * POLAR: polardb_admin is an internal database only accessible to
	 * superuser. Internal business can be done in polardb_admin without
	 * affecting normal users' business.
	 */
...
usage(const char *progname)
{
	...
	/* POLAR: parameters manage */
	printf(_("      --deploy-mode=[cloud|apclocal|apcshared]\n"
			 "                            choose the deployment mode of PolarDB\n"));
	/* POLAR end */
...
/* POLAR: parameters manage, TDE */
static void
check_deploy_mode(const char **deploy_mode)
{
	const char *const *p;

	if (*deploy_mode == NULL)
	{
		printf(_("PolarDB no use deploy mode\n"));
		return;
	}

	for (p = valid_deploy_modes; *p; p++)
	{
		if (strcmp(*deploy_mode, *p) == 0)
		{
			if (strcmp(*deploy_mode, "apcshared") == 0)
			{
				*deploy_mode = pg_strdup("cloud");
				printf(_("PolarDB deploy mode \"apcshared\" is mapped to \"cloud\""));
			}

			printf(_("PolarDB deploy in \"%s\" mode\n"),
				   *deploy_mode);
			return;
		}
	}

	pg_log_error("invalid deploy-mode \"%s\" for PolarDB", *deploy_mode);
...
setup_data_file_paths(void)
{
	...
	if (deploy_mode != NULL)
#ifdef POLAR_TODO
		set_input(&polar_settings_file, psprintf("polar_settings.conf.%s.%s.sample", polar_compatibility_mode, deploy_mode));
#else
		set_input(&polar_settings_file, psprintf("polar_settings.conf.%s.%s.sample", "pg", deploy_mode));
#endif
	...
	if (polar_settings_file != NULL)
		check_input(polar_settings_file);
...
create_data_directory(void)
{
	...
	switch ((ret = pg_check_dir(pg_data)))
	{
		...
		case 1:
			...
			if (polar_chmod(pg_data, pg_dir_create_mode) != 0)
...
create_xlog_or_symlink(void)
{
	...
	if (xlog_dir)
	{
		...
		switch ((ret = pg_check_dir(xlog_dir)))
		{
			...
			case 1:
				...
				if (polar_chmod(xlog_dir, pg_dir_create_mode) != 0)
					pg_fatal("could not change permissions of directory \"%s\": %m",
							 xlog_dir);
				else
					check_ok();
	...
	else
	{
		...
		if (polar_mkdir(subdirloc, pg_dir_create_mode) < 0)
			pg_fatal("could not create directory \"%s\": %m",
					 subdirloc);
	}
...
initialize_data_directory(void)
{
	...
	for (i = 0; i < lengthof(subdirs); i++)
	{
		 ...
		 */
		if (polar_mkdir(path, pg_dir_create_mode) < 0)
	...
	/*
	 * POLAR: In order to avoid problems caused by certain gucs like
	 * default_with_rowids = on taking effect during initdb,
	 * polar_settings.conf is generated according to template at the end of
	 * initialize_data_directory.
	 */
	setup_polar_settings(true);
}
