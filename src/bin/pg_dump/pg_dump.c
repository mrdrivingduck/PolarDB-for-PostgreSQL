...
/* POLAR: is polar_advisor schema or not */
#define IS_POLAR_ADVISOR_SCHEMA(schema_name) (strcmp(schema_name, "polar_advisor") == 0)
...
dumpNamespace(Archive *fout, const NamespaceInfo *nspinfo)
{
	...
	if (nspinfo->create)
	{
		...
		/*
		 * POLAR: polar_advisor schema contains the objects for maintenance
		 * window and vacuum. Sometimes it's already created in the target
		 * database by super_pg client in manager container, but sometimes
		 * not. In order to make pg_restore run successfully no matter
		 * polar_advisor schema exists in target db or not, we should add IF
		 * NOT EXISTS option.
		 */
		appendPQExpBuffer(q, "CREATE SCHEMA %s%s;\n",
						  IS_POLAR_ADVISOR_SCHEMA(nspinfo->dobj.name) ? "IF NOT EXISTS " : "", qnspname);
	}
	else
	{
...
dumpFunc(Archive *fout, const FuncInfo *finfo)
{
	...
	/* POLAR: see comment in dumpNamespace() */
					  IS_POLAR_ADVISOR_SCHEMA(finfo->dobj.namespace->dobj.name) ? "OR REPLACE " : "",
					  keyword,
					  fmtId(finfo->dobj.namespace->dobj.name),
...
dumpTableSchema(Archive *fout, const TableInfo *tbinfo)
{
	...
	else
	{
		...
		appendPQExpBuffer(q, "CREATE %s%s%s %s",
						  ...
						  reltypename,
		/* POLAR: see comment in dumpNamespace() */
						  IS_POLAR_ADVISOR_SCHEMA(tbinfo->dobj.namespace->dobj.name) ? " IF NOT EXISTS" : "",
						  qualrelname);
...
dumpIndex(Archive *fout, const IndxInfo *indxinfo)
{
	...
	if (!is_constraint)
	{
		...
		/*
		 * POLAR: see comment in dumpNamespace(). Do not add IF NOT EXISTS
		 * option in CREATE INDEX command because the command is generated by
		 * pg_get_indexdef() and we cannot add into it.
		 */
		if (IS_POLAR_ADVISOR_SCHEMA(indxinfo->dobj.namespace->dobj.name))
			appendPQExpBuffer(q, "DROP INDEX IF EXISTS %s;\n", qqindxname);
		/* Plain secondary index */
		appendPQExpBuffer(q, "%s;\n", indxinfo->indexdef);
...
dumpConstraint(Archive *fout, const ConstraintInfo *coninfo)
{
		...
		coninfo->contype == 'x')
	{
		...
		/*
		 * POLAR: see comment in dumpNamespace(). Do not add IF NOT EXISTS
		 * option in ADD CONSTRAINT command because that grammar is invalid.
		 */
		if (IS_POLAR_ADVISOR_SCHEMA(coninfo->dobj.namespace->dobj.name))
		{
			appendPQExpBuffer(q, "ALTER %sTABLE ONLY %s\n", foreign,
							  fmtQualifiedDumpable(tbinfo));
			appendPQExpBuffer(q, "    DROP CONSTRAINT IF EXISTS %s;\n",
